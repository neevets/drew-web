...
const hasAdminPermissions = (guild) => {
...
const renderAuthPage = (user, guilds) => {
  ...
  const manageableGuilds = guilds.filter((guild) => guild.owner || hasAdminPermissions(guild));
  const totalGuilds = manageableGuilds.length;
  const ownerGuilds = manageableGuilds.filter((guild) => guild.owner).length;
  const adminGuilds = manageableGuilds.filter((guild) => hasAdminPermissions(guild)).length;
    ...
    <main class="dashboard-shell oauth-grid">
      ...
      <section class="dashboard-card oauth-guilds">
        ...
        <div class="oauth-guilds-list">
          ${manageableGuilds.map(renderGuildCard).join("") || '<p class="oauth-empty">No tienes servidores donde seas admin u owner.</p>'}
        </div>
      </section>
    </main>
